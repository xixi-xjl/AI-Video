<template>
  <div class="bg-bg-section rounded-2xl border border-border shadow-lg overflow-hidden h-full flex flex-col">
        <!-- 标签页导航 -->
        ...
        <!-- 内容区域 -->
        <div class="p-5 sm:p-6 min-h-[200px] flex-1 overflow-y-auto">
          <!-- 加载状态 -->
          ...
          <div v-show="activeTab === 'summary'">
            <div v-if="needLoginPrompt && !summaryText" class="flex flex-col items-center justify-center py-16">
              <svg class="w-14 h-14 mb-4 text-primary/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
              <p class="text-text-primary font-medium mb-1">登录后体验 AI 视频总结</p>
              <p class="text-sm text-text-muted mb-4">免费用户每日 3 次，VIP 无限使用</p>
              <button @click="emit('need-login')" class="px-6 py-2.5 rounded-xl bg-primary text-white text-sm font-medium cursor-pointer hover:bg-primary-dark transition-colors">
                登录 / 注册
              </button>
            </div>
            <div
              v-if="summaryText"
              class="prose prose-slate prose-base max-w-none summary-prose"
            ...
            <!-- 免费用户剩余次数提示 -->
            <div v-if="quotaInfo && quotaInfo.remaining >= 0 && !loading" class="mt-4 p-3 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-between">
              <span class="text-base text-text-secondary">
                今日剩余 AI 总结次数：<strong class="text-text-primary">{{ quotaInfo.remaining }}</strong> / {{ quotaInfo.limit }}
              </span>
              ...
              <div class="flex items-center justify-between mb-4">
                <div class="text-sm text-text-secondary">
                  ...
                  <span v-if="subtitleData.language" class="ml-2 px-2 py-0.5 bg-primary-light text-primary rounded-full text-xs">
                    {{ subtitleTypeLabel }} · {{ subtitleData.language }}
                  </span>
                </div>
                <div class="flex items-center gap-3">
                  ...
                  <div class="relative" ref="subtitleDropdownRef">
                    ...
                    <div
                      ...
                      class="absolute right-0 top-full mt-1 bg-bg-section rounded-lg shadow-xl border border-border py-1 z-10 min-w-[120px]"
                    >
                      <button
          ...
          <div v-show="activeTab === 'mindmap'">
            <div v-if="mindmapMarkdown" class="relative">
              ...
              <div
                ...
                class="mindmap-wrapper w-full border border-border rounded-xl bg-white/[0.06] overflow-hidden"
                :class="isFullscreen ? 'mindmap-fullscreen' : 'min-h-[300px]'"
              >
          ...
          <div v-show="activeTab === 'create'">
            <div v-if="!props.user" class="flex flex-col items-center justify-center py-16 text-text-muted">
              <p class="text-sm mb-3">登录后可使用 AI 内容创作工具</p>
              <button @click="emit('need-login')" class="px-5 py-2 rounded-xl bg-primary text-white text-sm cursor-pointer">立即登录</button>
            </div>
            <div v-else-if="!subtitleData.has_subtitle && !loading" class="flex flex-col items-center justify-center py-16 text-text-muted">
              <p class="text-sm">请先生成视频总结以获取字幕内容</p>
            </div>
            <div v-else class="space-y-4">
              <p class="text-sm text-text-secondary">基于视频字幕，一键生成多种 AI 应用场景内容：</p>
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <button
                  v-for="scene in contentScenarios"
                  :key="scene.key"
                  @click="generateScene(scene.key)"
                  :disabled="contentLoading"
                  :class="[
                    'p-3 rounded-xl border text-left transition-all cursor-pointer',
                    activeScenario === scene.key
                      ? 'border-primary bg-primary-light'
                      : 'border-border-light hover:border-primary hover:bg-bg-section',
                    contentLoading && activeScenario !== scene.key ? 'opacity-50' : ''
                  ]"
                >
                  <div class="text-lg mb-1">{{ scene.icon }}</div>
                  <div class="text-xs font-medium text-text-primary">{{ scene.label }}</div>
                </button>
              </div>
              <div v-if="contentLoading" class="flex items-center gap-2 text-sm text-text-muted">
                <span class="w-4 h-4 border-2 border-primary/30 border-t-primary rounded-full animate-spin"></span>
                AI 正在生成{{ contentScenarios.find(s => s.key === activeScenario)?.label }}...
              </div>
              <div v-if="activeScenario === 'flashcards' && flashcards.length" class="space-y-2">
                <div
                  v-for="(card, idx) in flashcards"
                  :key="idx"
                  class="p-4 rounded-xl border border-border-light bg-bg-section"
                >
                  <div class="text-xs text-primary font-mono mb-1" v-if="card.timestamp">{{ card.timestamp }}</div>
                  <div class="text-sm font-medium text-text-primary mb-1">Q: {{ card.question }}</div>
                  <div class="text-sm text-text-secondary">A: {{ card.answer }}</div>
                </div>
              </div>
              <div
              ...
              ></div>
              <div v-if="contentText && !contentLoading" class="flex gap-2">
                <button
                  @click="copyContent"
                  class="text-xs px-3 py-1.5 rounded-lg border border-border-light hover:border-primary text-primary cursor-pointer"
                >
                  复制内容
                </button>
              </div>
            ...
            <div class="space-y-4">
                  ...
                  <div
                    :class="[
                      ...
                      msg.role === 'user'
                        ...
                        : 'bg-white/[0.08] text-text-primary rounded-bl-md border border-border'
                    ]"
                  >
                    <div v-if="msg.role === 'assistant'" class="chat-prose prose prose-invert prose-base max-w-none" v-html="renderMarkdown(msg.content)"></div>
              ...
              <div class="flex gap-2 pt-3 border-t border-border-light">
                <input
                  ...
                  type="text"
                  placeholder="输入你的问题..."
                  class="flex-1 h-11 px-4 rounded-xl border border-border bg-white/[0.06] text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all"
...
import { summarizeVideo, chatWithVideo, generateContent, getCachedSummary, fetchSubtitles } from '../api/summarize.js'
...
const subtitleTypeLabel = computed(() => {
  const t = subtitleData.value.subtitle_type
  if (t === 'manual') return '人工字幕'
  if (t === 'whisper') return 'Whisper 语音识别'
  return '自动字幕'
})
...
const needLoginPrompt = ref(false)

const contentScenarios = [
  { key: 'study_notes', label: '学习笔记', icon: '📚' },
  { key: 'xiaohongshu', label: '小红书文案', icon: '📱' },
  { key: 'wechat_article', label: '公众号大纲', icon: '📰' },
  { key: 'flashcards', label: '记忆闪卡', icon: '🃏' },
]
...
function getSubtitleTextForAI() {
  const segs = subtitleData.value.segments
  if (!segs?.length) return subtitleData.value.full_text || ''
  return segs.map((seg) => {
...
async function tryLoadCache() {
  try {
    const cached = await getCachedSummary(props.videoUrl)
    if (!cached?.summary_text) return false
    summaryText.value = cached.summary_text
    mindmapMarkdown.value = cached.mindmap_md || ''
    if (cached.subtitle?.has_subtitle) {
      subtitleData.value = cached.subtitle
    }
    return true
  } catch {
    return false
...
async function generateScene(scenario) {
  ...
  const text = getSubtitleTextForAI()
  if (!text.trim()) {
    alert('请先生成视频总结以获取字幕')
    return
  }
  ...
  try {
    await generateContent({
      ...
      content: (data) => {
        try { contentText.value += JSON.parse(data) } catch { contentText.value += data }
      },
      done: () => {
        contentLoading.value = false
        if (scenario === 'flashcards') {
          try {
            const parsed = JSON.parse(contentText.value.trim())
            flashcards.value = Array.isArray(parsed) ? parsed : []
          } catch {
            const match = contentText.value.match(/\[[\s\S]*\]/)
            if (match) {
              try { flashcards.value = JSON.parse(match[0]) } catch {}
            }
          }
        }
      },
      error: (data) => {
        contentLoading.value = false
        try {
          alert(JSON.parse(data).message || '生成失败')
        } catch {
          alert('生成失败')
        }
      },
    ...
    contentLoading.value = false
    if (err.message === 'NEED_LOGIN') emit('need-login')
    else alert(err.message)
  }
}

function copyContent() {
  const text = activeScenario.value === 'flashcards'
    ? flashcards.value.map((c, i) => `${i + 1}. Q: ${c.question}\n   A: ${c.answer}`).join('\n\n')
    : contentText.value
...
async function startSummarize() {
  if (!props.user) {
    needLoginPrompt.value = true
    loading.value = false
    return
  }
  needLoginPrompt.value = false
  loading.value = true
  summaryText.value = ''
    ...
    await summarizeVideo(props.videoUrl, 'zh', {
      subtitle: (data) => {
        try {
          ...
          if (subtitleData.value.has_subtitle) {
            loadingMessage.value = subtitleData.value.subtitle_type === 'whisper'
              ? '语音识别完成，AI 正在分析视频内容...'
              : 'AI 正在分析视频内容...'
          }
      ...
      asr: (data) => {
        try {
          ...
          loadingMessage.value = parsed.message || '正在使用 Whisper 语音识别...'
        } catch {
          loadingMessage.value = '正在使用 Whisper 语音识别...'
        }
      },
      summary: (data) => {
...
async function sendQuestion() {
  ...
  try {
    await chatWithVideo(
      props.videoUrl,
      question,
      getSubtitleTextForAI() || subtitleData.value.full_text || '',
      {
        asr: (data) => {
          try {
            const parsed = JSON.parse(data)
            aiMessage.content = '⏳ ' + (parsed.message || '正在 Whisper 语音识别...')
          } catch {
            aiMessage.content = '⏳ 正在 Whisper 语音识别...'
          }
          scrollChatToBottom()
        },
        subtitle_ready: (data) => {
    ...
    if (err.message === 'NEED_LOGIN') {
      emit('need-login')
      aiMessage.content = '请先登录后使用 AI 问答'
    } else {
...
watch(() => props.user, async (user) => {
  if (user && !summaryText.value && !loading.value) {
    needLoginPrompt.value = false
    const loaded = await tryLoadCache()
    if (!loaded) startSummarize()
  }
...
.chat-prose :deep(h1), .chat-prose :deep(h2), .chat-prose :deep(h3), .chat-prose :deep(h4) {
  color: var(--color-text-primary);
  font-weight: 600;
...
t.mindmap-wrapper :deep(.markmap-link) {
	  stroke: rgba(168, 85, 247, 0.4) !important;
	}
	.mindmap-wrapper :deep(g > circle) {
