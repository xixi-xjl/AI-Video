<template>
  <div class="bg-bg-section rounded-2xl border border-border shadow-lg overflow-hidden h-full">
    <!-- 视频信息头部 -->
    <div class="flex flex-col gap-5 p-5 sm:p-6">
      <div class="relative w-full aspect-video rounded-xl overflow-hidden bg-black/30">
        <video
        ...
        />
        <img
          v-else-if="video.thumbnail && !thumbnailFailed"
          :src="thumbnailUrl"
          :alt="video.title"
    ...
    <div class="border-t border-border-light px-5 sm:px-6 py-5">
      ...
      <div class="grid grid-cols-1 gap-2.5">
        ...
        >
          <div class="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center"
            :class="selectedFormat === fmt.format_id ? 'bg-primary text-white' : 'bg-white/5 text-text-muted'">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="mt-6 flex flex-col items-stretch gap-3">
        <button
          ...
          class="w-full inline-flex items-center justify-center gap-2 h-12 px-10 rounded-full bg-gradient-to-r from-purple-600 to-cyan-500 hover:opacity-90 text-white font-medium text-base transition-all disabled:opacity-50 disabled:cursor-not-allowed glow-btn cursor-pointer"
        >
...
const previewVideoUrl = computed(() => {
  const direct =
    props.video.preview_url ||
    props.video.formats?.find((fmt) => fmt._direct_url)?._direct_url
  if (!direct) return ''
  return '/api/proxy/video?url=' + encodeURIComponent(direct)
})
