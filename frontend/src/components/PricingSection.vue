  ...
  <section id="pricing" class="py-16 sm:py-20 bg-bg-section" aria-labelledby="pricing-heading">
    <div class="max-w-5xl mx-auto px-4 sm:px-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
        <!-- 免费版 -->
        <div class="bg-white/[0.04] rounded-2xl border border-border p-7 flex flex-col">
          <div class="mb-6">
          ...
          <button
            ...
            :class="user ? 'bg-white/5 cursor-default' : 'hover:bg-white/5 cursor-pointer'"
        ...
        <div class="relative bg-gradient-to-br from-primary to-blue-600 rounded-2xl p-7 flex flex-col text-white overflow-hidden">
          ...
          <div class="relative">
            ...
            <button
              ...
              :disabled="vipLoading"
              class="w-full h-11 rounded-full bg-white text-primary text-sm font-semibold hover:bg-white/90 transition-colors shadow-lg cursor-pointer disabled:opacity-70 disabled:cursor-wait"
            >
              {{ vipLoading ? '正在跳转支付...' : (user?.is_vip ? '续费 VIP' : '立即开通 VIP') }}
            </button>
          </div>
        </div>
      </div>
...
const props = defineProps({
  ...
  vipLoading: { type: Boolean, default: false },
})
