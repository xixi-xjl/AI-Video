    ...
    <main class="flex-1">
      ...
      <section v-if="videoData" class="py-4 sm:py-6 bg-bg-main">
        <div class="max-w-7xl mx-auto px-4 sm:px-6">
          <button
            @click="videoData = null; currentUrl = ''"
            class="mb-4 inline-flex items-center gap-2 text-sm text-text-secondary hover:text-text-primary transition-colors cursor-pointer"
          >
      ...
      <HowToSection />
      <ComparisonSection />
      <PricingSection :user="currentUser" :vip-loading="vipLoading" @open-vip="handleOpenVip" @need-login="showAuthModal('login')" />
      <PlatformSection />
    </main>
    <AppFooter />
...
async function handleOpenVip() {
  ...
  try {
    ...
    if (!checkout_url) {
      alert('未获取到支付链接，请检查 Stripe 配置')
      return
    }
    window.location.href = checkout_url
  } catch (err) {
    const msg = err.response?.data?.detail || err.message || '创建支付失败，请稍后重试'
