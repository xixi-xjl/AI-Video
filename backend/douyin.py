...
class DouyinParser:
    ...
    def parse(self, url: str) -> dict:
        ...
        video_id, resolved_url = self._resolve_video_context(share_url)
    ...
    def _has_embedded_video_id(self, url: str) -> bool:
        """链接中已包含视频 ID（搜索页 modal_id、/video/ 路径等）"""
        parsed = urlparse(url)
        query = parse_qs(parsed.query)
        for key in ("modal_id", "item_ids", "group_id", "aweme_id"):
            values = query.get(key)
            if values and re.search(r"\d{8,24}", values[0]):
                return True
        return bool(re.search(r"/video/(\d{8,24})", parsed.path))

    def _resolve_video_context(self, share_url: str) -> tuple[str, str]:
        """
        解析视频 ID 与用于回退抓取的稳定页面地址。
        搜索页/带 modal_id 的链接直接提取 ID，跳过重定向以加快解析。
        """
        if self._has_embedded_video_id(share_url):
            video_id = self._extract_video_id(share_url)
            return video_id, f"https://www.iesdouyin.com/share/video/{video_id}/"
